...
void gridSearchTargeted(const TerrainMesh& tmesh,
    ...
    std::list<point>path=path_manager(newMap, point(newMap.landSite.x, newMap.landSite.y), point(newMap.vantagePoints.at(1).x, newMap.vantagePoints.at(1).y));
    std::list<point>::iterator it;
    for (it = path.begin(); it != path.end(); ++it) {
        drawPoint(it->x, it->y, 4.0, img );//{128, 0, 128}
    }
